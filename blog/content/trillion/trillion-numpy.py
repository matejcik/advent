...
class NonDeterministicALU:
    ...
    def deduplicate(self):
        ...
        known_states = {}
        cursor = 0
        for current in range(self.num_states):
            state = states[current]
            key = tuple(state[REG_SELECTOR])
            if key in known_states:
                self.update_minmax(known_states[key], current)
            else:
                known_states[key] = current
                states[cursor] = state
                cursor += 1
        self.num_states = cursor

    def update_minmax(self, state1, state2):
        self.states[state1]["min"] = self.states["min"][[state1, state2]].min()
        self.states[state1]["max"] = self.states["max"][[state1, state2]].max()
